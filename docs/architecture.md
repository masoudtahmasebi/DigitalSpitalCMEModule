...
1. **`AuthGuard`** reads `X-DS-Project`, resolves the project binding through a
   ...
   _That project's_ realm is the load-bearing part. The API holds **no**
   Keycloak configuration of its own — the issuer is a column on the project
   row, read per request — which is what lets one installation serve several
   customers with separate identity providers, and what keeps a customer's
   Keycloak outage away from the admin console entirely (ADR-0012). Three
   deployment-wide Keycloak variables were declared in the config schema for a
   long time and read by nothing; they are gone (P17-02).
...
### The server owns its credentials

`~/ds-education/config.env` is written once by a human — a domain, a project, an
ACME mailbox. `~/ds-education/secrets.env` is generated by the first deploy and
never regenerated: two database passwords, the Postgres superuser password and
the KMS key, `openssl rand -base64 32` each. Nobody reads them and nobody types
them, so nobody ever needs to have seen one.

Both live outside the git clone, so a `git checkout` cannot touch a credential.
GitHub holds an SSH key and three strings that identify a host — nothing that
unlocks anything.

The consequence for backups is the one thing to remember: **the KMS key is part
of the backup.** A dump restored without it brings back rows whose `_enc`
columns are permanently unreadable, because there is no plaintext fallback.

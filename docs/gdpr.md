...
| Certificate delivery address            | `enrolments.delivery_email`                                                | Where this participant's Teilnahmebescheinigung is sent, when that differs from `users.email` (P183-01). Set by the physician on their own enrolment or by an operator in the support panel. **Not** the account address: for a portal account that is also the sign-in credential, so the two are deliberately separate columns. Cleared by `erase_subject`.                                                                                                 |
...
| Evaluation answers                      | `evaluation_responses`                                                     | **Not** required for the Anerkennung (P206-01, S35): this file asserted it was, on the same unsourced claim the console carried, and the client confirmed on 07.09.2026 that it is not mandatory. Collected only where the customer has authored questions, so the basis is the customer's own interest in feedback rather than Art. 6(1)(c). Free-text answers are the one place a physician may type something about a patient — treated accordingly in §5. |
...
| Recording watch time and quiz results                | Art. 6(1)(c) with the Fortbildungsordnung and the Anerkennungsbescheid — the Kammer's conditions are what make these mandatory rather than optional                |
...
`db/migrations/0052_enrolment_delivery_email.sql` — and before it by `0041`
(the archived PDF and the Anschrift) and `0024` (the name parts) — invoked
through `apps/api/src/subject-erasure.ts`.

This line has been wrong twice: it named `0024` while `0041` was the amendment
in force. That is CLAUDE.md §11 rule 14, and on this particular sentence it is
worse than untidy — somebody adding a column and reading it would extend the
wrong function.
...
statement as the composed name they were derived from. 0052 is the most recent:
`delivery_email` is a second place a subject's e-mail address lives, and the
migration adds it to the very statement this paragraph warns about. Its
integration test asserts the _send stops_, not only that the column is null,
because a column that survived would also mean the certificate still went out.
...
| Attested address, and the certificate delivery address      | The fact the certificate was issued, and to which course                    |

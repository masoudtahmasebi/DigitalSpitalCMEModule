...
/**
 * The checkpoint a learner is credited in whole units of (S30, P157-01).
 *
 * The client's rule, in his words:
 *
 * > if someone reaches the minute 2, then we are sure they watched the minute
 * > 1, if someone reaches minute 33, they have watched until minute 32 at least
 *
 * The same minute `resumePosition` already rewinds to, asserted equal in
 * `watch.test.ts` — so where a learner comes back to and what they are credited
 * for are one granularity rather than two that can disagree (§9.11).
 */
export const WATCH_CHECKPOINT_SEC = 60;

/**
 * The union, plus the whole minutes the furthest point reached implies.
 *
 * ## Why this is not "max position wins"
 *
 * `CLAUDE.md` §4 invariant 5 forbids crediting by furthest position, and the
 * reason it gives is exact: *"a max-position implementation makes any gate
 * trivially skippable by dragging the scrub bar."* That was true when it was
 * written and it is what S30 had to answer.
 *
 * It stopped being true with P154-01. The forward seek ceiling is now half a
 * second above the watched edge — an order of magnitude below the smallest
 * forward control, with a test pinning that ratio — so the playhead can pass
 * unwatched content **only by playing it**. `playerSeekLimit` advances
 * `reachedSec` on playback and never on a seek, for the same reason. With the
 * scrub bar unable to move the ceiling, "the furthest point reached" stops
 * being a client claim and becomes a consequence of playback.
 *
 * So this credits **completed minutes strictly below that point**, and nothing
 * above it. A learner at 33:20 is credited through 33:00; a learner at 0:47 is
 * credited nothing they did not actually watch, because no minute is finished.
 * Everything past the furthest point is left exactly as reported — which is the
 * half of invariant 5 that has not changed and must not.
 *
 * ## Why whole minutes rather than seconds
 *
 * The gaps this exists for are sampling artefacts — a throttled `timeupdate`, a
 * flush that failed, a boundary landing between two samples — and they are
 * fractions of a second to a few seconds. A minute is coarse enough to absorb
 * all of them and fine enough that skipping a minute is still skipping. It is
 * also the granularity the product already speaks in: the resume point rewinds
 * to the start of the containing minute, and a learner who comes back to 33:00
 * and is told 32:58–33:00 is missing has been given a chore rather than an
 * answer.
 *
 * ## What it deliberately does not do
 *
 * It does not rewrite what was reported. The stored union stays the evidence of
 * what was played; this derives what is credited from it, so the two can always
 * be compared and the rule can be withdrawn without having destroyed anything.
 */
export function creditedWatchedSegments(
  segments: readonly WatchedSegment[],
): readonly WatchedSegment[] {
  const reached = maxWatchedPosition(segments);
  const completed = Math.floor(reached / WATCH_CHECKPOINT_SEC) * WATCH_CHECKPOINT_SEC;
  if (completed <= 0) return mergeWatchedSegments(segments);
  return mergeWatchedSegments([...segments, { startSec: 0, endSec: completed }]);
}
